...
public class SpatialGrid : MonoBehaviour
{
	...
	private Dictionary<GridEntity, Tuple<int, int>> lastPositions = new Dictionary<GridEntity, Tuple<int, int>>();
	...
	private void Awake()
	{
		...
		var ents = RecursiveWalker(transform)
				  .Select(n => n.GetComponent<GridEntity>())
				  .Where(n => n != null);
	...
	public IEnumerable<GridEntity> Query(Vector3 aabbFrom, Vector3 aabbTo, Func<Vector3, bool> filterByPosition)
	{
		...
		return cells
			  ...
			  .Where(n => filterByPosition(n.transform.position));
	}
	...
	void OnDestroy()
	{
		var ents = RecursiveWalker(transform).Select(n => n.GetComponent<GridEntity>())
											 .Where(n => n != null);
	...
	private void OnDrawGizmos()
	{
		...
		if (!activatedGrid)
		{
			var allElems = new List<GridEntity>();
